...
write.table(new,"svm_smote300_150_fit_pectin_TunedModel_cor07_tree500_exp9Fourth_withoutImpFilter.xls",sep="\t",col.names=NA)
 
importance <- importance(svmModel$finalModel)
plot(importance)
...
library(pROC)
rf.probs <- predict(svmModel, testing,type="prob")
head(rf.probs)
rf.ROC <- roc( 
               testing$type,
               rf.probs[,"pectin"],

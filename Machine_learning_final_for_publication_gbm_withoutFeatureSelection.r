...
write.table(new,"gbm_smote300_150_fit_pectin_TunedModel_cor07_tree500_exp9Fourth__withoutImpFilter.xls",sep="\t",col.names=NA)
 
importance <- importance(gbmModel$finalModel)
plot(importance)
...
library(pROC)
rf.probs <- predict(gbmModel, testing,type="prob")
head(rf.probs)
rf.ROC <- roc( 
               testing$type,
               rf.probs[,"pectin"],
